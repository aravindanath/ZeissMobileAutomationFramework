...
public class BaseTest extends BaseTestFunctions {
	...
	public void initData() //Inputs values into the HashMaps testData and testScenario from the Excel file.
	{
		...
		int rc= DataReadWriteLibrary.getExcelRowCount(path, "Patient");
		for (int i = 0; i <= rc; i++)
		{
			if (!DataReadWriteLibrary.getExcelData(path, "Patient",i,0).equals(" "))
			{
				testData.put(DataReadWriteLibrary.getExcelData(path, "Patient",i,0), DataReadWriteLibrary.getExcelData(path, "Accounts",i,2));
				testScenario.put(DataReadWriteLibrary.getExcelData(path, "Patient",i,0), DataReadWriteLibrary.getExcelData(path, "Accounts",i,1));
			}
		}
